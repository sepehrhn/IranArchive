...
const supabaseUrl = process.env.IRANTRIAL_SUPABASE_URL;
const supabaseAnonKey = process.env.IRANTRIAL_SUPABASE_ANON_KEY;
const iranTrialHeroesUrl = 'https://irantrial.org/fa/heroes';
const pageSize = 1000;
...
if (!supabaseUrl || !supabaseAnonKey) {
  console.error(
    'Set IRANTRIAL_SUPABASE_URL and IRANTRIAL_SUPABASE_ANON_KEY before running this sync.'
  );
  process.exit(1);
}
...
function buildDescription(remote) {
  ...
  if (remote.status === 'Killed') {
    ...
    return `IranTrial lists ${remote.name} among those killed${details.length ? ` ${details.join(' ')}` : ''}.`;
  }
  ...
  return `IranTrial lists ${remote.name} as detained${qualifier}${details.length ? ` ${details.join(' ')}` : ''}.`;
}
...
function toVictimYaml(remote) {
  ...
  const record = {
    irantrial_ids: [remote.id],
    irantrial_categories: [remote.category],
    photo: [],
    name: remote.name,
    ...
    irantrial_updated_at: remote.updatedAt
  };
...
function indexRecord(indexes, wrapper) {
  for (const id of arrayValue(wrapper.data.irantrial_ids).map(asString).filter(Boolean)) {
    indexes.byIranTrialId.set(id, wrapper);
  }
  ...
  const references = Array.isArray(wrapper.data.irantrial_source_references)
    ? wrapper.data.irantrial_source_references
    : wrapper.data.irantrial_source_reference
      ? [wrapper.data.irantrial_source_reference]
      : [];
...
function exactSignals(remote, wrapper) {
  ...
  const sourceOverlap = [...a.sourceUrls].some(
    (url) => url !== iranTrialHeroesUrl && b.sourceUrls.has(url)
  );
  return {
...
function chooseMatch(remote, indexes) {
  if (indexes.byIranTrialId.has(remote.id)) {
    return {
      wrapper: indexes.byIranTrialId.get(remote.id),
      method: 'irantrial_id'
    };
  }
  if (
...
function isIranTrialManagedRecord(wrapper) {
  return (
    /^vic-2026-[0-9a-f]{12}\.ya?ml$/u.test(wrapper.file) &&
    arrayValue(wrapper.data.irantrial_ids).length > 0
  );
}
...
function mergeRemote(wrapper, remote) {
  const data = wrapper.data;
  let changed = false;
  const managedRecord = isIranTrialManagedRecord(wrapper);
  const previousName = asString(data.name);

  const existingIds = Array.isArray(data.irantrial_ids)
    ? data.irantrial_ids
    : data.irantrial_id
      ? [data.irantrial_id]
      : [];
  ...
  if (JSON.stringify(ids) !== JSON.stringify(existingIds)) {
    data.irantrial_ids = ids;
    delete data.irantrial_id;
    changed = true;
  }

  const existingCategories = Array.isArray(data.irantrial_categories)
    ? data.irantrial_categories
    : [];
  ...
  if (JSON.stringify(categories) !== JSON.stringify(existingCategories)) {
    data.irantrial_categories = categories;
    changed = true;
  }
  ...
  if (
    managedRecord &&
    asString(data.description).startsWith(`IranTrial lists ${previousName}`) &&
  ...
  const currentIranTrialUpdatedAt = asString(data.irantrial_updated_at);
  if (
    remote.updatedAt &&
    (!currentIranTrialUpdatedAt ||
      Date.parse(remote.updatedAt) > Date.parse(currentIranTrialUpdatedAt))
  ) {
    data.irantrial_updated_at = remote.updatedAt;
    changed = true;
  }
  if (remote.sourceReference) {
    const existingReferences = Array.isArray(data.irantrial_source_references)
      ? data.irantrial_source_references
      : data.irantrial_source_reference
        ? [data.irantrial_source_reference]
        : [];
    ...
    if (JSON.stringify(references) !== JSON.stringify(existingReferences)) {
      data.irantrial_source_references = references;
      delete data.irantrial_source_reference;
      changed = true;
    }
  }
...
function newVictimId(remote, usedFiles) {
  const digest = crypto
    .createHash('sha256')
    .update(`irantrial:${remote.table}:${remote.id}`)
    .digest('hex');
  ...
  throw new Error(`Unable to allocate a unique victim ID for IranTrial ${remote.id}`);
}
...
console.log('Fetching verified IranTrial fallen and detainee records...');
...
remotes.sort((a, b) => {
  const completeness = (record) =>
    [
      ...
      ...record.sourceUrls.filter((url) => url !== iranTrialHeroesUrl)
...
const matchedIranTrialFiles = new Set();
...
for (const remote of remotes) {
  ...
  if (wrapper) {
    ...
    else matchedIranTrialFiles.add(wrapper.file);
  ...
  if (remote.sourceReference) {
    data.irantrial_source_references = [remote.sourceReference];
  }
...
for (const wrapper of localRecords) {
  if (wrapper.deleted || !isIranTrialManagedRecord(wrapper)) continue;
  const ids = arrayValue(wrapper.data.irantrial_ids).map(asString).filter(Boolean);
  if (ids.some((id) => validRemoteIds.has(id))) continue;
  if (isPlausibleIndividualName(wrapper.data.name)) continue;
  wrapper.deleted = true;
  wrapper.deleteReason = 'IranTrial source row has no usable individual name';
...
const duplicateIranTrialIds = [];
for (const wrapper of activeRecords) {
  const ids = Array.isArray(wrapper.data.irantrial_ids)
    ? wrapper.data.irantrial_ids
    : [];
  for (const id of ids) {
    if (idOwners.has(id) && idOwners.get(id) !== wrapper.file) {
      duplicateIranTrialIds.push([id, idOwners.get(id), wrapper.file]);
    } else {
...
if (duplicateIranTrialIds.length > 0) {
  console.error('IranTrial IDs assigned to more than one victim record:');
  console.error(duplicateIranTrialIds.slice(0, 20));
  process.exit(1);
}
...
const summary = {
  ...
  matched_new_irantrial_files: matchedIranTrialFiles.size,
  ...
  retained_created_records: activeRecords.filter(
    (record) => record.origin === 'irantrial'
  ).length,
  ...
  unique_irantrial_ids: idOwners.size,
  duplicate_irantrial_ids: duplicateIranTrialIds.length,
  remaining_likely_duplicate_fingerprints: remainingLikelyDuplicates.length,
  records_with_multiple_irantrial_ids: activeRecords.filter(
    (record) =>
      Array.isArray(record.data.irantrial_ids) && record.data.irantrial_ids.length > 1
  ).length,
  maximum_irantrial_ids_on_one_record: Math.max(
    0,
    ...activeRecords.map((record) =>
      Array.isArray(record.data.irantrial_ids) ? record.data.irantrial_ids.length : 0
    )
  ),
  largest_irantrial_id_groups: activeRecords
    .filter(
      (record) =>
        Array.isArray(record.data.irantrial_ids) &&
        record.data.irantrial_ids.length > 1
    )
    .sort((a, b) => b.data.irantrial_ids.length - a.data.irantrial_ids.length)
    .slice(0, 10)
    .map((record) => ({
      file: record.file,
      name: record.data.name,
      persian_name: record.data.persian_name,
      count: record.data.irantrial_ids.length
    }))
};
