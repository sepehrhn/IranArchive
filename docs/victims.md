...
## external_source Synchronization

Use `scripts/sync-external_source-victims.mjs` to compare the registry with external_source's
verified public Eternal Wall records. The script runs as a dry run unless
`--write` is supplied.

```bash
external_source_SUPABASE_URL="https://..." \
external_source_SUPABASE_ANON_KEY="..." \
node scripts/sync-external_source-victims.mjs
```
...
- records stable provenance in `external_source_ids`, `external_source_categories`, and
  `external_source_source_references`;
- normalizes provinces to `data/provinces.json`;
- merges only strong identity matches and rejects duplicate external_source IDs.
