<template>
  <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
    ...
    <div>
      ...
      <div class="space-y-4">
        <div v-for="source in primarySources" :key="source.id" :id="`source-${source.id}`" class="p-4 bg-surface-0 dark:bg-surface-900 border border-surface-200 dark:border-surface-700 rounded-lg hover:shadow-md transition-shadow">
          <a :href="source.url" target="_blank" rel="noopener noreferrer" class="block group">
            ...
            <div class="text-xs text-surface-500 mb-2 truncate">{{ source.url }}</div>
          </a>
        ...
        <p v-if="primarySources.length === 0" class="text-sm text-surface-500 italic">No primary sources listed.</p>
      </div>
    </div>
    ...
    <div>
      ...
      <div class="space-y-2">
         <ul class="list-disc list-inside space-y-2">
             <li v-for="source in secondarySources" :key="source.id" :id="`source-${source.id}`" class="text-sm text-surface-700 dark:text-surface-300">
                <a :href="source.url" target="_blank" rel="noopener noreferrer" class="text-primary-600 hover:underline">
                    {{ source.label }}
                </a>
                <span v-if="source.publisher" class="text-surface-500 mx-1">- {{ source.publisher }}</span>
             </li>
         </ul>
         <p v-if="secondarySources.length === 0" class="text-sm text-surface-500 italic">No secondary sources listed.</p>
      </div>
    </div>
  </div>
</template>
